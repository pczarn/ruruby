...
module Inline
  class Rust
    ...
    ##
    # Default Rust to Ruby and Ruby to Rust type map

    TYPE_MAP = {
       'int' => [ -> n { "#{n}.to_int()" },  'FromPrimitive::from_int'],
       'i64' => [ -> n { "#{n}.to_i64().unwrap()" }, 'FromPrimitive::from_i64' ],
    }
   ...
   p sig, @types, ARG
    ...
    def generate(src, options={})
      ...
      if expand_types then
        prefix = "extern fn #{function_name}("
        if arity <= MAGIC_ARITY then
          ...
          #prefix += "max: int"
         p signature['args']
          prefix += signature['args'].map { |arg, type| ", _#{arg}: Value"}.join
        end
        prefix += ") -> Value {\n"
        prefix += signature['args'].map { |arg, type|
          "  let #{arg}: #{type} = #{ruby2rust(type, "_#{arg}")};\n"
        }.join
        ...
        unless return_type == "void" then
          ...
          result.gsub!(/return\s+([^\;\}]+)/) do
            "return #{rust2ruby(return_type)}(#{$1}).unwrap()"
          end
        else
          result.sub!(/\s*\}\s*\Z/, "\nreturn Qnil;\n}")
        end
    ...
    def generate_ext
      ...
      if @include_ruby_first
        @inc.unshift %{#![crate_type = "dylib"] extern crate ruruby; use ruruby::value::Value;}
      else
        @inc.push %{#![crate_type = "dylib"] extern crate ruruby; use ruruby::value::Value;}
      end
      ...
      ext << "  pub fn Init_#{module_name}() {"
      ext << "    #![no_mangle] use ruruby::{const_get, define_method, intern}; use ruruby::raw::rb_cObject;"
      ext << "    let mut c: Value = rb_cObject;"

      # TODO: use rb_class2path
      # ext << "    VALUE c = rb_path2class(#{@mod.name.inspect});"
      ext << @mod.name.split("::").map { |n|
        "    c = const_get(c, intern(\"#{n}\"));"
      }.join("\n")
      ...
      @sig.keys.sort.each do |name|
        ...
        if singleton then
          if method_name == 'allocate' then
            ...
            ext << "    rb_define_alloc_func(c, #{name});"
            next
          end
          method << "    rb_define_singleton_method(c, \"#{method_name}\", "
        else
          method << "    define_method(c, \"#{method_name}\", "
        end
        method << "#{name} as *const (), #{arity});"
        ext << method
      end
...
VALUE #{method}_equals(VALUE value) {
  ...
  return value;
    ...
    def ruby2rust(type, varname)
      raise ArgumentError, "Unknown type #{type.inspect}" unless @type_map.has_key? type
      obj = @type_map[type].first
    ...
    def build
      ...
      unless so_exists and File.mtime(rb_file) < File.mtime(so_name) then
        ...
        src_name = "#{Inline.directory}/#{module_name}.rs"
        ...
        if recompile then
          ...
          sane = ! windoze
          pwd = `pwd`.strip
          cmd = [ "rustc -O -C link-args=-Wl,-rpath=#{pwd}/target/ --crate-type=dylib",
                  flags,
                  # "-L#{pwd}/target/x86_64-unknown-linux-gnu/lib/",
                  "-L #{pwd}/target/",
    ...
    def extern_crate(crate)
      @inc << "extern crate #{crate};"
    end
